...
describe('proAllows', () => {
  it('gives a built feature to a paying account and to nobody else', () => {
    for (const feature of ['day-plan', 'companion'] as const) {
  ...
  it('refuses everything still unbuilt, paid or not', () => {
    // These three are names on a list. When a phase finishes one it joins BUILT
    // and moves to the test above, which is the point of asserting it here.
    for (const feature of ['calendar', 'culture', 'intimacy'] as const) {
...
describe('anythingBuilt', () => {
  it('says there is something to sell now', () => {
    ...
    expect(PRO_FEATURES.filter(isBuilt)).toEqual(['day-plan', 'companion'])
  })
})
