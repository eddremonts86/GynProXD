...
one card still marked `Coming`, four things noted while building the rest, and a
third tier that has been proposed but not decided.
...
## 6. Week 1 of a plan that starts mid-week

`assemblePlan` maps each training day to its weekday offset from the start date,
so a plan begun on a Tuesday shows "Week 1 · Monday" six days later while
Wednesday is tomorrow. It affects every plan, not just an adopted one, and it
reads as a bug even though the dates are all correct.

Small, visible, and unrelated to anything above. Probably the cheapest
